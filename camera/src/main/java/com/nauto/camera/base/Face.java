...
public final class Face {
    ...
    public static void copy(android.hardware.camera2.params.Face[] in, Face[] out) {
        if (in != null) {
    ...
    public static void copy(android.hardware.camera2.params.Face in, Face out) {
        Rect bounds = in.getBounds();
        if (bounds != null) {
            out.mBounds.top = bounds.top;
            out.mBounds.bottom = bounds.bottom;
            out.mBounds.left = bounds.left;
            out.mBounds.right = bounds.right;
        }
        out.mScore = in.getScore();
        out.mId = in.getId();
        Point p = in.getLeftEyePosition();
        if (p != null) {
            out.mLeftEye.x = p.x;
            out.mLeftEye.y = p.y;
        }
        p = in.getRightEyePosition();
        if (p != null) {
            out.mRightEye.x = p.x;
            out.mRightEye.y = p.y;
        }
        p = in.getMouthPosition();
        if (p != null) {
            out.mMouth.x = p.x;
            out.mMouth.y = p.y;
        }
    ...
    public String toString() {
        return String.format("{\"r\":[%.3f,%.3f,%.2f,%.2f], \"s\":%d}",
                (double)mBounds.left / 1920.,
                (double)mBounds.top / 1080.,
                (double)mBounds.width() / 1920.,
                (double)mBounds.height() / 1080.,
                mScore);
    }
